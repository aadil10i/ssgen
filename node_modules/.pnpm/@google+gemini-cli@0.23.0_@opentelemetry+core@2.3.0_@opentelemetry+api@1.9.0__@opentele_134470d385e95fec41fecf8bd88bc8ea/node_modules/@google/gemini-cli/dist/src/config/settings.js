...
export class LoadedSettings {
    constructor(system, systemDefaults, user, workspace, isTrusted, migratedInMemoryScopes) {
        this.system = system;
...
export function loadSettings(workspaceDir = process.cwd()) {
    ...
    }, isTrusted, migratedInMemoryScopes);
}
export function migrateDeprecatedSettings(loadedSettings, extensionManager) {
    const processScope = (scope) => {
        const settings = loadedSettings.forScope(scope).settings;
        if (settings.extensions?.disabled) {
            debugLogger.log(`Migrating deprecated extensions.disabled settings from ${scope} settings...`);
