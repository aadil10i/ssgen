...
import { UserTierId, AuthType, TerminalQuotaError, makeFakeConfig, RetryableQuotaError, PREVIEW_GEMINI_MODEL, ModelNotFoundError, DEFAULT_GEMINI_MODEL, DEFAULT_GEMINI_FLASH_MODEL, } from '@google/gemini-cli-core';
    ...
    describe('Fallback Handler Logic', () => {
        ...
        describe('Interactive Fallback', () => {
            ...
            it('should handle race conditions by stopping subsequent requests', async () => {
                ...
                let result2;
                await act(async () => {
                    result2 = await handler('gemini-pro', 'gemini-flash', new TerminalQuotaError('pro quota 2', mockGoogleApiError));
                });
            ...
            for (const { description, error } of testCases) {
                it(`should handle ${description} correctly`, async () => {
                    ...
                    expect(mockHistoryManager.addItem).toHaveBeenCalledTimes(1);
                    const lastCall = mockHistoryManager.addItem.mock
                        .calls[0][0];
            ...
            it('should handle ModelNotFoundError correctly', async () => {
                ...
                const message = request.message;
                expect(message).toBe(`It seems like you don't have access to gemini-3-pro-preview.
...
To disable gemini-3-pro-preview, disable "Preview features" in /settings.`);
    ...
    describe('handleProQuotaChoice', () => {
        ...
        it('should resolve intent to "retry_always" and add info message on continue', async () => {
            ...
            // Check for the "Switched to fallback model" message
            expect(mockHistoryManager.addItem).toHaveBeenCalledTimes(1);
            const lastCall = mockHistoryManager.addItem.mock.calls[0][0];
            expect(lastCall.type).toBe(MessageType.INFO);
