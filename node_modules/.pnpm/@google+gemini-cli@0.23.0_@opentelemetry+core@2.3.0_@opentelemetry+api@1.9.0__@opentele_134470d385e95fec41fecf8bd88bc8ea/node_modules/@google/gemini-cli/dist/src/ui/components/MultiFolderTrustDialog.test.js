...
import { MultiFolderTrustDialog, MultiFolderTrustChoice, } from './MultiFolderTrustDialog.js';
...
describe('MultiFolderTrustDialog', () => {
    beforeEach(() => {
        ...
        mockedRadioButtonSelect.mockImplementation((props) => (_jsx("div", { "data-testid": "RadioButtonSelect", ...props })));
    });
    it('renders the dialog with the list of folders', () => {
        const folders = ['/path/to/folder1', '/path/to/folder2'];
        const { lastFrame } = render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders }));
        expect(lastFrame()).toContain('Do you trust the following folders being added to this workspace?');
    ...
    it('calls onComplete and finishAddingDirectories with an error on escape', async () => {
        ...
        render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders }));
        ...
        expect(mockFinishAddingDirectories).toHaveBeenCalledWith(mockConfig, mockAddItem, [], [
    ...
    it('adds directories to workspace context when "Yes" is chosen', async () => {
        const folders = ['/path/to/folder1', '/path/to/folder2'];
        render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders, trustedDirs: ['/already/trusted'] }));
        const { onSelect } = mockedRadioButtonSelect.mock.calls[0][0];
        await act(async () => {
    ...
    it('adds directories to workspace context and remembers them as trusted when "Yes, and remember" is chosen', async () => {
        ...
        expect(mockAddDirectory).toHaveBeenCalledWith('/path/to/folder1');
        expect(mockSetValue).toHaveBeenCalledWith('/path/to/folder1', TrustLevel.TRUST_FOLDER);
        expect(mockFinishAddingDirectories).toHaveBeenCalledWith(mockConfig, mockAddItem, ['/path/to/folder1'], []);
        expect(mockOnComplete).toHaveBeenCalled();
    });
    ...
    it('shows an error message and completes when config is missing', async () => {
        const folders = ['/path/to/folder1'];
        render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders, config: null }));
        const { onSelect } = mockedRadioButtonSelect.mock.calls[0][0];
        await act(async () => {
            onSelect(MultiFolderTrustChoice.YES);
        });
    ...
    it('collects and reports errors when some directories fail to be added', async () => {
        ...
        render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders, errors: ['initial error'] }));
        const { onSelect } = mockedRadioButtonSelect.mock.calls[0][0];
        await act(async () => {
            onSelect(MultiFolderTrustChoice.YES);
        });
        ...
        expect(mockFinishAddingDirectories).toHaveBeenCalledWith(mockConfig, mockAddItem, ['/path/to/good'], ['initial error', "Error adding '/path/to/error': Test error"]);
        expect(mockOnComplete).toHaveBeenCalled();
    });
});
//# sourceMappingURL=MultiFolderTrustDialog.test.js.map