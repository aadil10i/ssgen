...
describe('ide-installer', () => {
    ...
    describe('VsCodeInstaller', () => {
        function setup({ ide = IDE_DEFINITIONS.vscode, existsResult = false, execSync = () => '', platform = 'linux', } = {}) {
        ...
        describe('install', () => {
            ...
            it('installs the extension using code cli', async () => {
                ...
                await installer.install();
                expect(child_process.spawnSync).toHaveBeenCalledWith('code', [
                    '--install-extension',
            ...
            it('installs the extension using code cli on windows', async () => {
                ...
                await installer.install();
                expect(child_process.spawnSync).toHaveBeenCalledWith('C:\\Program Files\\Microsoft VS Code\\bin\\code.cmd', [
                    '--install-extension',
                    'google.gemini-cli-vscode-ide-companion',
                    '--force',
...
describe('AntigravityInstaller', () => {
    function setup({ execSync = () => '', platform = 'linux', } = {}) {
        vi.spyOn(child_process, 'execSync').mockImplementation(execSync);
    ...
    it('installs the extension using the alias', async () => {
        ...
        const result = await installer.install();
        expect(result.success).toBe(true);
        expect(child_process.spawnSync).toHaveBeenCalledWith('agy', [
            '--install-extension',
            'google.gemini-cli-vscode-ide-companion',
            '--force',
        ], { stdio: 'pipe', shell: false });
