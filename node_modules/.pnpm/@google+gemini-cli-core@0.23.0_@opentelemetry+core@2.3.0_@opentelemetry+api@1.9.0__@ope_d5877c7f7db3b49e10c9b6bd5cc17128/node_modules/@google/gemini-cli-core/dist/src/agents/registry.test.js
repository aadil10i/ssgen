...
import { DEFAULT_GEMINI_FLASH_LITE_MODEL, GEMINI_MODEL_ALIAS_AUTO, PREVIEW_GEMINI_FLASH_MODEL, PREVIEW_GEMINI_MODEL, PREVIEW_GEMINI_MODEL_AUTO, } from '../config/models.js';
...
describe('AgentRegistry', () => {
    ...
    describe('initialize', () => {
        ...
        it('should log the count of loaded agents in debug mode', async () => {
            ...
            expect(debugLogSpy).toHaveBeenCalledWith(`[AgentRegistry] Initialized with ${agentCount} agents.`);
        });
        it('should use preview flash model for codebase investigator if main model is preview pro', async () => {
            ...
            const investigatorDef = previewRegistry.getDefinition('codebase_investigator');
            expect(investigatorDef).toBeDefined();
            expect(investigatorDef?.modelConfig.model).toBe(PREVIEW_GEMINI_FLASH_MODEL);
        });
        ...
        it('should use the model from the investigator settings', async () => {
            ...
            const investigatorDef = previewRegistry.getDefinition('codebase_investigator');
            expect(investigatorDef).toBeDefined();
            expect(investigatorDef?.modelConfig.model).toBe(DEFAULT_GEMINI_FLASH_LITE_MODEL);
        });
        it('should load agents from user and project directories with correct precedence', async () => {
            ...
            expect(registry.getDefinition('common-agent')?.description).toBe('Project version');
            expect(registry.getDefinition('project-only')).toBeDefined();
            expect(vi.mocked(tomlLoader.loadAgentsFromDirectory)).toHaveBeenCalledTimes(2);
        });
        it('should NOT load TOML agents when enableAgents is false', async () => {
            ...
            expect(vi.mocked(tomlLoader.loadAgentsFromDirectory)).not.toHaveBeenCalled();
        });
    ...
    describe('registration logic', () => {
        it('should register a valid agent definition', () => {
            ...
            expect(mockConfig.modelConfigService.getResolvedConfig({
        ...
        it('should reject an agent definition missing a name', () => {
            ...
            expect(debugWarnSpy).toHaveBeenCalledWith('[AgentRegistry] Skipping invalid agent definition. Missing name or description.');
        });
        ...
        it('should log overwrites when in debug mode', () => {
            ...
            expect(debugLogSpy).toHaveBeenCalledWith(`[AgentRegistry] Overriding agent 'MockAgent'`);
        });
        it('should not log overwrites when not in debug mode', () => {
            ...
            registry.testRegisterAgent(MOCK_AGENT_V1);
            registry.testRegisterAgent(MOCK_AGENT_V2);
            expect(debugLogSpy).not.toHaveBeenCalledWith(`[AgentRegistry] Overriding agent 'MockAgent'`);
        });
    ...
    describe('getToolDescription', () => {
        it('should return default message when no agents are registered', () => {
            expect(registry.getToolDescription()).toContain('No agents are currently available');
        });
        it('should return formatted list of agents when agents are available', () => {
            ...
            expect(description).toContain('Delegates a task to a specialized sub-agent');
            expect(description).toContain('Available agents:');
            expect(description).toContain(`- **${MOCK_AGENT_V1.name}**: ${MOCK_AGENT_V1.description}`);
            expect(description).toContain(`- **AnotherAgent**: Another agent description`);
        });
    });
});
