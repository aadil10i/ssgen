 ...
 */
import { DiagLogLevel, diag, trace, context, metrics, propagation, } from '@opentelemetry/api';
...
let activeTelemetryEmail;
export function isTelemetrySdkInitialized() {
    return telemetryInitialized;
}
export function bufferTelemetryEvent(fn) {
...
export async function initializeTelemetry(config, credentials) {
    ...
    if (config.getTelemetryUseCollector() && config.getTelemetryUseCliAuth()) {
        debugLogger.error('Telemetry configuration error: "useCollector" and "useCliAuth" cannot both be true. ' +
    ...
    if (useDirectGcpExport) {
        debugLogger.log('Creating GCP exporters with projectId:', gcpProjectId, 'using', credentials ? 'provided credentials' : 'ADC');
 ...
 */
export async function flushTelemetry(config) {
...
export async function shutdownTelemetry(config, fromProcessExit = true) {
