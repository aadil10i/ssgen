...
import { getErrorMessage, FatalAuthenticationError, FatalCancellationError, } from '../utils/errors.js';
...
import { writeToStdout, createWorkingStdio, writeToStderr, } from '../utils/stdio.js';
import { enableLineWrapping, disableMouseEvents, disableKittyKeyboardProtocol, enterAlternateScreen, exitAlternateScreen, } from '../utils/terminal.js';
    ...
    if (credentials) {
        ...
        try {
            ...
            if (token) {
                ...
                await triggerPostAuthCallbacks(credentials);
                return client;
            }
        }
        catch (error) {
...
async function authWithUserCode(client) {
    try {
        ...
        try {
            ...
            writeToStderr('Failed to authenticate with authorization code:' +
        ...
        writeToStderr('Failed to authenticate with user code:' + getErrorMessage(err) + '\n');
        debugLogger.error('Failed to authenticate with user code:', getErrorMessage(err));
        return false;
    }
}
